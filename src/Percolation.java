...
public class Percolation {
    ...
    {
        // lets connect all cells which are open and neighbours
        WeightedQuickUnionUF field = new WeightedQuickUnionUF(size*size+2);
        for (int i=1; i<=size; i++)
            for (int j=1; j<=size; j++)
            {
                if (isOpen(i,j))
                {
                    // Check if left, right, up and down neighbours are open
                    // that connect them with current node (union them)
